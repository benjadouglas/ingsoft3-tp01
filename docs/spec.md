...
Una skill (`handoff-html`, en `skill/`) con dos clientes detrás de un mismo punto de entrada, elegidos según dónde corre el agente:

- **Claude Code**: un script que loopea el long-poll, reintenta ante error de red, y termina imprimiendo la acción cuando llega. El agente lo lanza en un Monitor persistente, su turno termina, y el harness lo despierta con el resultado. Si la espera muere (Monitor cerrado, sesión reabierta), volver a correr `wait` recupera la acción pendiente: el servidor no la consume hasta la versión siguiente. El script lee el id de sesión del entorno y título y directorio del log local (`~/.claude/projects/*/<id>.jsonl`).
- **T3 Code** (Codex o Cursor por debajo; el shell del agente muere al terminar el turno, así que no puede esperar): el script solo publica. Resuelve el thread por la API HTTP de T3 (el único thread con turno activo cuyo proyecto es este repo), guarda `harness=t3code`, id, título y el link directo al thread (`session_url`), y deja un `sesion.json` local. Un daemon en la máquina del usuario (`skill/handoff-html/scripts/bridge`, LaunchAgent) hace el long-poll por él y, cuando llega la acción, le escribe al thread un mensaje con la referencia a la skill y el mismo JSON de `wait`. Si el thread está ocupado, rebota la acción (`POST .../acciones/rebotar`) y el usuario la vuelve a mandar cuando termine. T3 no se modifica: el bridge usa el pairing oficial (bearer de ~30 días, `orchestration:read operate`).

Codex y Cursor no son harness propios: se usan a través de T3 Code. El visor muestra, cuando el turno es del agente, un botón "Reanudar": para T3 Code un link "Abrir en T3 Code"; para Claude Code el comando de CLI (`cd <dir> && claude --resume <id>`) o el título a buscar en la GUI; para cualquier otro harness un prompt para pegar en la conversación original, que retoma con `wait`. La key vive en `BORRADOR_TOKEN` / `~/.config/borrador/token`, nunca en un repo.
...
### Cliente local de Borrador

La skill `handoff-html` usa `skill/handoff-html/scripts/client/borrador.mjs` (Node ≥ 20). `borrador config --url <api> --app-url <visor>` guarda la configuración; no hay `defaults.sh`. Las variables de entorno tienen prioridad. Toda integración con T3 usa `skill/handoff-html/scripts/bridge/t3code.mjs`. Cada sesión guarda un único `sesion.json` y su `plan.html`; `plan_id` y el marcador `approved` anteriores se migran al guardar. La lógica de rebote se mantiene.
