...
Publish the plan discussed in this conversation to Borrador and wait for the user to review it. All HTTP goes through `<base-dir>/scripts/borrador.sh`, where `<base-dir>` is this skill's base directory (the one reported when the skill was loaded; never guess or hardcode another path). **Always call it by that absolute path from the user's repo. Never `cd` into the skill directory**: the script takes the project name from the directory it runs in, and a plan published from the wrong place is filed under the wrong project for good. It needs `curl`, `jq`, and `BORRADOR_TOKEN` or `~/.config/borrador/token` (API defaults to `http://localhost:3000`, viewer to `http://localhost:5173`). The script remembers which plan belongs to this repo: you never handle ids.
