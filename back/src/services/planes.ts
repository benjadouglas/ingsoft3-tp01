...
export async function obtenerOCrearProyecto(userId: string, nombre: string) {
  const [fila] = await db
    ...
    .onConflictDoUpdate({ target: [proyecto.userId, proyecto.nombre], set: { nombre } })
...
export async function publicarPlan(
  ...
  const dueño = await db.query.proyecto.findFirst({
    where: and(eq(proyecto.id, input.proyectoId), eq(proyecto.userId, userId)),
  });
  if (!dueño) return;
...
export async function obtenerPlan(userId: string, id: string): Promise<Plan | undefined> {
  const [fila] = await db
    .select({ plan })
    .from(plan)
...
export async function listarPlanes(userId: string) {
  return db
    .select({ id: plan.id, titulo: plan.titulo, proyecto: proyecto.nombre, version: plan.version, actualizadoEl: plan.actualizadoEl })
    .from(plan)
    .innerJoin(proyecto, eq(plan.proyectoId, proyecto.id))
