...
export async function obtenerOCrearProyecto(userId: string, nombre: string) {
  const [fila] = await db
    ...
    .onConflictDoUpdate({ target: [project.userId, project.name], set: { name: nombre } })
...
export async function publicarPlan(
  userId: string,
  input: { proyectoId: string; titulo: string; contenidoHtml: string; sessionId?: string },
) {
  const dueño = await db.query.project.findFirst({
    where: and(eq(project.id, input.proyectoId), eq(project.userId, userId)),
  });
  if (!dueño) return;
  return db.transaction(async (tx) => {
    const [creado] = await tx
      .insert(plan)
      .values({ projectId: input.proyectoId, title: input.titulo, sessionId: input.sessionId })
      .returning({ id: plan.id });
    await tx.insert(version).values({ planId: creado!.id, number: 1, htmlContent: input.contenidoHtml });
    return { id: creado!.id, version: 1 };
  });
}

/** HTML de la versión actual del plan, si pertenece al usuario. */
export async function obtenerHtmlActual(userId: string, id: string): Promise<string | undefined> {
  const [fila] = await db
