...
export const planStateEnum = pgEnum("plan_state", ["user_turn", "agent_turn", "approved"]);
...
export const comment = pgTable(
  ...
  {
    id: uuid("id").primaryKey().defaultRandom(),
    versionId: uuid("version_id").notNull().references(() => version.id, { onDelete: "cascade" }),
...
export const action = pgTable(
  "action",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    planId: uuid("plan_id").notNull().references(() => plan.id, { onDelete: "cascade" }),
    versionId: uuid("version_id").notNull().references(() => version.id),
    type: actionTypeEnum("type").notNull(),
    consumed: boolean("consumed").notNull().default(false),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    consumedAt: timestamp("consumed_at", { withTimezone: true }),
  },
  (t) => [uniqueIndex("action_pending_per_plan_uidx").on(t.planId).where(sql`${t.consumed} = false`)],
);
